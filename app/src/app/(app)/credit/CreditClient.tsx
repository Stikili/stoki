...
export default function CreditClient({
  ...
  function handleClearDebt(debtor: Debtor) {
    if (!confirm(`Mark full R${debtor.totalOwed.toFixed(2)} from ${debtor.name} as paid?`)) return
    startTransition(async () => {
      await clearDebtAction(debtor.id, debtor.totalOwed)
      toast(`${debtor.name} — debt cleared`)
    })
  }
  ...
  return (
    <>
      ...
      >
        ...
        <p className={`text-xs font-semibold uppercase tracking-widest mb-2 ${totalOutstanding > 0 ? 'text-danger/70' : 'text-brand/70'}`}>Total Outstanding</p>
          ...
          <div className="relative rounded-t-3xl p-6 pb-10" style={sheetStyle}>
            ...
            <div className="flex flex-col gap-3">
              <input
                ...
                step="0.01"
                min="0.01"
                max={settleDebtor.totalOwed}
                placeholder={`Amount (max R${settleDebtor.totalOwed.toFixed(2)})`}
                value={settleAmount}
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setSettleAmount(settleDebtor.totalOwed.toFixed(2))}
                  className="flex-1 text-xs font-semibold py-3 rounded-xl"
                  style={{ background: 'rgba(255,255,255,0.05)', color: '#5a7a94', border: '1px solid rgba(255,255,255,0.1)' }}
                >
                  Full (R{settleDebtor.totalOwed.toFixed(2)})
                </button>
                <button
                  onClick={() => setSettleAmount((settleDebtor.totalOwed / 2).toFixed(2))}
                  className="flex-1 text-xs font-semibold py-3 rounded-xl"
                  style={{ background: 'rgba(255,255,255,0.05)', color: '#5a7a94', border: '1px solid rgba(255,255,255,0.1)' }}
                >
                  Half (R{(settleDebtor.totalOwed / 2).toFixed(2)})
                </button>
              </div>
              <button
                onClick={handlePartialSettle}
                disabled={isPending || !settleAmount}
